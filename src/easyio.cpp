...
char *read_string(char const *prompt, char *def) {
    ...
    for (int timeout = 0; timeout <= 5; timeout++) {
        ...
        esito = (short)scanstring(stringa);
        switch (esito) {
            case EASYIO_DEFAULT:
                return (def);
            case EASYIO_OKAY:
